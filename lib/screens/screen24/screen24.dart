...
class Screen24 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    }  else if (currentModel is Model24_0) {
      return _buildContent(context, currentModel.name);
    }  else if (currentModel is Model24_1) {
      return _buildContent(context, currentModel.name);
    }  else if (currentModel is Model24_2) {
      return _buildContent(context, currentModel.name);
    } 
    return _buildContent(context, '');
  }
  ...
  Widget _buildContent(BuildContext context, String name) {
    return  TextButton(onPressed: () => showScreen(context, model: Model25_0(name)), child: const Text('Screen25'));
  }
